...
class FileIdentifierHashUpdate extends AbstractUpdate {
	...
	public function checkForUpdate(&$description) {
		...
		$unmigratedStorageCount = $this->db->exec_SELECTcountRows(
			'uid',
			'sys_file_storage',
			'driver = "Local" AND ( configuration NOT LIKE "%caseSensitive%" OR configuration NOT LIKE "%unicodeNormalization%" )'
		);

		$unicodeNormalization = isset($GLOBALS['TYPO3_CONF_VARS']['SYS']['UTF8filesystem'])
			? (int) $GLOBALS['TYPO3_CONF_VARS']['SYS']['UTF8filesystem']
			: 0;
		$constraint = ' AND configuration %s LIKE "%%<unicodeNormalization>%s</unicodeNormalization>%%"';
		$unmigratedStorageCount += $this->db->exec_SELECTcountRows(
			'uid',
			'sys_file_storage',
			'driver = "Local"' .
				sprintf($constraint, '', '%') .
				sprintf($constraint, 'NOT', '%'.$unicodeNormalization.'%')
		);
