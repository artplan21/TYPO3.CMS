...
return array(
	...
	'SYS' => array(
		...
		'UTF8filesystem' => 0,					// Integer (0, 1, 2, 3, 4, 5): If > 0 then TYPO3 uses utf-8 to store file names. This allows for accented Latin letters as well as any other non-latin characters like Cyrillic and Chinese. Values > 1 additionally activate unicode-normalization for filepaths, where you can choose one of the four normalization-forms: NFD (2), NFKD(3), NFC(4) and NFKC(5). As NFC is the defacto worldwide standard and preserves some bytes compared to NFD, it's advisable to stick to it. Hint: Mac OSX's HFS+ filesystem uses NFD to store paths. It provides significant faster sorting algorithms. Even if you choose something else than NFD here HFS+ Filesystems will always use NFD and decompose path-strings if needed. For further details see \TYPO3\CMS\Core\Charset\UnicodeNormalizer.
		'unicodeNormalization' => 0,				// Integer (0, 1, 2, 3, 4, 5): Determines default unicode-normalization of BE out- and input, unless overridden by "CONFIG.unicodeNormalization". The default 0 completely disables any unicode-normalization attempts, in the frontend. If set to 1, it is currently disabled too, but future/upcoming implementations may choose a proper normalization automatically, ie. based upon the operating-system, file-system or other relevant factors. Values > 1 directly refer to one of the four normalization-forms: NFD (2), NFKD(3), NFC(4) and NFKC(5). Additionally they will override any upcoming auto-detection completly. Hint: The W3C recommends NFC for HTML5. For further details see \TYPO3\CMS\Core\Charset\UnicodeNormalizer.
		'unicodeNormalizeInputs' => '',			// String: Activates unicode-normalization for some or all global user-input-replated arrays, like the prominent $_POST array. If enabled the the algorithm ensures for all elements in listed globals with type string to be well-formed and normalized UTF-8. A comma-seperated list of the named globals without their prefix "$_…", like "FILES", "ENV", "GET", "POST", "COOKIE", "SERVER" or "REQUEST". The value "ALL" refers to all of them at once and the empty "" value disables any normalization attempts. Only active if the related charset, ie. "CONFIG.metaCharset", is set to "utf8".
	...
	'FE' => array( // Configuration for the TypoScript frontend (FE). Nothing here relates to the administration backend!
		...
		'unicodeNormalization' => 0,				// Integer (0, 1, 2, 3, 4, 5): Determines default unicode-normalization of FE out- and input, unless overridden by "CONFIG.unicodeNormalization". The default 0 completely disables any unicode-normalization attempts, in the frontend. If set to 1, it is currently disabled too, but future/upcoming implementations may choose a proper normalization automatically, ie. based upon the operating-system, file-system or other relevant factors. Values > 1 directly refer to one of the four normalization-forms: NFD (2), NFKD(3), NFC(4) and NFKC(5). Additionally they will override any upcoming auto-detection completly. Only active if the related charset, ie. "CONFIG.metaCharset", is set to "utf8". Hint: The W3C recommends NFC for HTML5. For further details see \TYPO3\CMS\Core\Charset\UnicodeNormalizer.
		'unicodeNormalizeInputs' => '',				// String: Activates unicode-normalization for some or all global user-input-replated arrays, like the prominent $_POST array. If enabled the the algorithm ensures for all elements in listed globals with type string to be well formed and normalized UTF-8. A comma-seperated list of the named globals without their prefix "$_…", like "FILES", "ENV", "GET", "POST", "COOKIE", "SERVER" or "REQUEST". The value "ALL" refers to all of them at once and the empty "" value disables any normalization attempts. Only active if the related charset, ie. "CONFIG.metaCharset", is set to "utf8".
		'redirectToUtf8EncodedRequestUriIfNeeded' => FALSE, // Boolean: Activates redirecting to a unicode-normalized request uri in FE if the requested url is not well-formed UTF-8. Default is FALSE.
